...
class MainWindow(tk.Tk):
    ...
    def _build(self) -> None:
        ...
        ttk.Button(self._update_bar, text=t("Update Now"), command=self._do_update, width=20).pack(
            side="right"
        )
        self._update_url = ""
        ...
        self._nb.add(self._main_tab, text=f"  {t('Main')}  ")
        self._nb.add(self._history_tab, text=f"  {t('History')}  ")
    ...
    def _start_tray(self) -> None:
        try:
            ...
            menu = pystray.Menu(
                pystray.MenuItem(t("Open"), self._tray_open, default=True),
                pystray.MenuItem(t("Quit"), self._tray_quit),
            )
    ...
    def _on_update_result(self, result: Optional[tuple]) -> None:
        if result:
            ...
            self._update_lbl.config(text=t("Update v{version} available").format(version=new_version))
